...
async function loadSnipeList() {
  ...
  const newMintStr = findAddedElement(snipeList, newSnipeList)
  if(newMintStr) {
    console.log("New token is added")
    try {
      const mint = new PublicKey(newMintStr)
      await buyInList(mint, wallet)
    } catch (error) {
      
    }
  }

  snipeList = newSnipeList
}

function findAddedElement(originalArray: string[], modifiedArray: string[]): string | undefined {
  // Function to find the element in array2 that is not in array1
  function findDifference(array1: string[], array2: string[]): string | undefined {
  ...
  // Determine which array is larger to identify the added element
  if (modifiedArray.length > originalArray.length) {
    return findDifference(originalArray, modifiedArray);
  } 
}
...
const buyInList = async (mint: PublicKey, mainKp: Keypair) => {
  processingToken = true
  try {
    const poolInfo = await formatAmmKeysById(solanaConnection, poolId.toBase58())
    if(!poolInfo){
      console.log("")
      return
    }
    const poolKeys = jsonInfo2PoolKeys(poolInfo)
    let tx = await getBuyTx(solanaConnection, mainKp, mint, NATIVE_MINT, Number(QUOTE_AMOUNT), poolId.toBase58(), poolKeys)
    if (tx == null) {
      console.log(`Error getting buy transaction`)
      return null
    }
    console.log("Volume bot bought")
    const txSig = await executeVersionedTx(tx)
...
export const executeVersionedTx = async (transaction: VersionedTransaction) => {
  ...
  if (confirmation.value.err) {
    ...
    console.log(`Confrimed transaction: https://solscan.io/tx/${signature}`)
  }
  return signature
}
